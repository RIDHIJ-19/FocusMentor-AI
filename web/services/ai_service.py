...
Ported from the desktop app's app/services/ai_service.py. Same logic;
adapted to work with plain dicts (this app's repository layer returns
dicts, not dataclasses) and to read the API key from web/settings.py
(env var only) instead of the desktop's QSettings/.env/registry chain.
...
from web.repository import STATUS_COMPLETED
...
class AIService:
    ...
    def generate_insight(self, recent_tasks: List[dict]) -> str:
        api_key = settings.get_api_key()
    ...
    def _local_insight(self, recent_tasks: List[dict]) -> str:
        if not recent_tasks:
        ...
        lines.append(
            "Add a free Groq API key (server env var) for personalized, AI-generated coaching feedback."
        )
        return " ".join(lines)
